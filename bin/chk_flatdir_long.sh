...
do
	...
	if [ `echo $OSTYPE | grep linux` != "" ] ; then 
		#TARGETDIR=`/usr/bin/python /home/cube/bin/date.py "$FORMAT" $DATESTRING "%Y/%m/%d" 2>/dev/null`
		TARGETDIR=`/home/cube/bin/bsddate "$FORMAT" $DATESTRING "%Y/%m/%d" 2>/dev/null`
	else
