...
Version="0.2"
#
# 2011-02-21  Norbert Flatinger, IPE
#
# - don't use a proxy server for file retrieval, so added option '--no-proxy';
#   using a proxy would retrieve some unwanted files generated by the proxy!
# - added some wget optins in comment
#
################################################################################
#
...
# --no-remove-listing	don't remove the temporary '.listing' file
# -r			retrieve files recursive
# -l depth		specify recursion maximum depth level
# -m			turn on options suitable for mirroring: '-r -N -l inf --no-remove-listing'
################################################################################
if [ -x $WGETPROG ] ; then
	# sync data
	$WGETPROG -m -nH -nd --no-proxy -P "$DEST" "$SRC"
	RETURN_VALUE=$?
	if [ $RETURN_VALUE != 0 ] ; then
