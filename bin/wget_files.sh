...
Version="0.1"
#
# 2011-02-18  Norbert Flatinger, IPE
#
# - this script is intended to use wget to synchronize files from a remote
...
################################################################################
# do synchronization with "wget"
# interesting options are:
# -b		go to background, log to 'wget-log'
# -o logfile	log to logfile
# -a logfile	append to logfile
# -q		quiet
# -N		turn on time-stamping
# -nd		don't create directory hierarchy when retrieving recursively
# -nH		don't create host-prefixed directories
# -P prefix	save files to destination directory prefix
# -r		retrieve files recursive
# -m		turn on options suitable for mirroring: '-r -N -l inf --no-remove-listing'
################################################################################
if [ -x $WGETPROG ] ; then
	# sync data
	$WGETPROG -m -nH -nd -P "$DEST" "$SRC"
	RETURN_VALUE=$?
	if [ $RETURN_VALUE != 0 ] ; then
